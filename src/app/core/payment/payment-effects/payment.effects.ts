...
// Payment Revoke
  PaymentRevoke$: Observable<Action> = createEffect(() => this.action$.pipe(
    ofType(actions.ActionTypes.EXPORT_ARCHIVE_REVOKE),
    map((action: actions.PaymentRevokeAction) => action.payload),
    switchMap(state => {
      return this.service.PaymentRevoke(state).pipe(
        switchMap(user => [new actions.PaymentRevokeSuccess(user)]),
        catchError(error => of(new actions.PaymentRevokeFail(error)))
      );
    })
  ));
