...
export class VendorRegComponent implements OnInit {
  ...
  lowerCaseError: any = 'ValidationsErrorMessage.lowerCaseError';
  upperCaseError: any = 'ValidationsErrorMessage.upperCaseError';
  numberError = 'ValidationsErrorMessage.numberError';
  specialCharError = 'ValidationsErrorMessage.specialCharacterError';
  ...
  onSubmit(): void {
    ...
    this.subscriptions.add(this.authSandbox.registerFailed$.subscribe(val => {
      if (val?.error?.message === "Please enter a valid OTP") {
        this.otpValidation = true;
      }
    }))
  }
