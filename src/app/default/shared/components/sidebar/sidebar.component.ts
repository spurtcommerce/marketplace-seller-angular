...
import { timeZoneArray } from '../../../../../../src/app/default/pages/component/my-account/personalised-setting/personalized-settings.constant';
...
export class SidebarComponent implements OnInit {
  ...
  ngDoCheck(){
    ...
    if(!['',null,undefined].includes(JSON.parse(localStorage.getItem('languageName')))){
      this.showLanguage =JSON.parse(localStorage.getItem('languageName'));
    }else{
  ...
  ngOnInit() {
    ...
    this.CommonService.profileAPi({}).subscribe((profile) => {
      if (profile && profile?.status == 1) {
        ...
        let timefilterval=this.timezone.find((vals)=>vals?.name==profile?.data?.personalizedSettings?.timeZone)
      
        localStorage.setItem('timeZone',JSON.stringify(timefilterval?.timeZone ||''));
      }
    })
  ...
  getSettings() {
    this.CommonService.doGetSettings({}).subscribe((vals) => {
      if (vals && vals?.status == 1) {
        let val = vals?.data
        localStorage.setItem('vendor-settings', JSON.stringify(vals.data[0]));
        localStorage.setItem('languageName', JSON.stringify(vals.data[0].storeLanguageName));
        if (val) {
          ...
          val?.forEach(element => {
            ...
            if (!['', null, undefined, "0"].includes(element?.sellerLogo2Path) && !['', null, undefined, "0"].includes(element.sellerLogo2)) {
            ...
            if (!['', null, undefined, "0"].includes(element?.sellerLogoPath) && !['', null, undefined, "0"].includes(element.sellerLogo)) {
              this.maxPostSellerImageUrl = this.imageUrl + '?path=' + element?.sellerLogoPath + '&name=' + element.sellerLogo + '&width=500&height=500';
            } else {
              this.maxPostSellerImageUrl = ''
            }
            ...
            if (!['', null, undefined].includes(this.sellerData?.personalizedSettings?.defaultLanguage) && this.sellerData?.personalizedSettings?.defaultLanguage != 0) {
              ...
              if (this.languageArray.length > 0) {
                ...
                if (!['', null, undefined].includes(selectLang?.code)) {
                  this.translate.setDefaultLang(selectLang?.code);
                  this.translate.use(selectLang?.code);
                }
                // this.showLanguage = selectLang?.name;
  ...
  addOnConfig() {
    ...
    this.subscriptions.add(this.commonSandbox.addOnConfig$.subscribe((val) => {
  ...
  getLanguageList() {
    ...
    this.CommonService.languageList1(params).subscribe((val) => {
      if (val && val.status == 1) {
        if (val) {
          this.languageArray = val?.data;
        }
      }
    })
  }
