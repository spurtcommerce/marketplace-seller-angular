...
import { timeZoneArray } from '../../pages/component/my-account/personalised-setting/personalized-settings.constant';
  ...
  getVendorProfile() {
    ...
    this.CommonService.profileAPi({}).subscribe((val) => {
      if (val && val.status == 1) {
        ...
        let timefilterval=this.timezone.find((vals)=>vals?.name==val?.data?.personalizedSettings?.timeZone)
