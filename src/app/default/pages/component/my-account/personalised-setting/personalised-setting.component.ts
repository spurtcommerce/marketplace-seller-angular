...
export class PersonalisedSettingComponent implements OnInit {
    ...
    if (token) {
      ...
      this.subscriptions.add(this.myShopSandbox.getPersonalizedSettings$.subscribe(val => {
        if (val?.data) {
          ...
          let timeZones = val?.data?.personalizedSettings?.timeZone ?? null;
          this.timeZone = this.zone.filter((val: any) => val?.name == timeZones)[0]?.name
          ...
          if (!['', null, undefined].includes(val.data?.personalizedSettings?.defaultLanguage) && val.data?.personalizedSettings?.defaultLanguage != 0) {
            ...
            this.timezonelocalValue =JSON.parse(localStorage.getItem('timeZone'));
  ...
  changeTimeZone(event) {
    this.timezonelocalValue = event.timeZone
  }
  submit() {
    const params: any = {};
    ...
    localStorage.setItem('timeZone', JSON.stringify(this.timezonelocalValue));
