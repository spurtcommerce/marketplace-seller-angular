...
export class ProductDetailsComponent implements OnInit {
    ...
    this.subscriptions.add(this.productsandbox.ProductUpdateDetails$.subscribe((val) => {
      if (val?.status == 1 && val?.data) {
        ...
        if (!['', null, undefined].includes(val?.data?.productVideo?.path)) {
          ...
          let datas = {
            image: val?.data?.productVideo.name,
            path: val?.data?.productVideo.path
          }
          this.videodetails = datas
  ...
  getDatasss() {
    ...
    let datas = {
      image: val?.videoDetails?.image,
      path: val?.videoDetails?.path
    }
    this.videodetails = datas
    if (val?.image) {
  ...
  formFields() {
    this.myForm = this.formBuilder.group({
      ...
      inventory: ['', [Validators.required, Validators.maxLength(7)]],
