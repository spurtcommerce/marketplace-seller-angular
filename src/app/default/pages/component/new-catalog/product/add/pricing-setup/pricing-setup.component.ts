...
export class PricingSetupComponent implements OnInit {
  ...
  onSubmit() {
    ...
    if (["", null, undefined].includes(this.valProductDetail.productName)) {
      ...
      // Filter out duplicate images where defaultImage = 1
      const filteredDataImage = this.valProductDetail.dummyImage.filter((item, index, self) =>
        item.defaultImage === 0 ||
        index === self.findIndex(i => i.image === item.image && i.defaultImage === 1)
      );
      ...
      if (this.DefaultPricemyForm.valid) {
        ...
        if (!['', null, undefined].includes(this.editId)) {
          ...
          this.productsandbox.Productedit$.subscribe((val)=>{
          ...
          this.subscribtion.add(this.productsandbox.ProductCreation$.subscribe((val: any) => {
            ...
            if (val?.status === 1) {
              ...
              if (val?.data?.productId && val?.status === 1) {
                this.subscribtion.add(this.seoServices.CreateProductSeo(data).subscribe((val) => {
                  if (val?.status === 1 && onjects?.productSpecifications?.length > 0) {
                    this.subscribtion.add(this.seoServices.Specification_Create(onjects).subscribe((val) => {
                      this.subscribtion.add(this.seoServices.attributeSlug(onjects?.productId).subscribe(val => {
                        if (val?.status === 1) {
                          this.resetServices();
