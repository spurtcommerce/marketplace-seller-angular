...
export class ArchiveOrdersComponent implements OnInit, AfterViewInit {
  ...
  exportOrder(): void {
    if (this.selectedDatas.length > 0) {
      ...
      this.selectedDatas.forEach((res: any) => {
 ...
 actionType(type: string): void {
  ...
  private closebutton(): void {
    this.archiveProductArray.forEach((val) => (
      val.selected = false
    ));
    this.reset();
  ...
  private reset(isChecked = false): void {
    this.archiveProductArray.forEach(val => val.checked = isChecked);
    this.selectedDatas = this.archiveProductArray.filter(val => val?.checked);
  }
